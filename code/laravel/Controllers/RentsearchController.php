...
class RentsearchController extends Controller {
	...
	public function index(Request $request) {
		...
		if(isset($_POST) && !empty($_POST)){
			...
			$sendData['city'] = @$_POST['search'];
		} else{
			$sendData['search'] = '';
			$sendData['city'] = session('city');
		}
		$sendData['search_input'] = $sendData['search'];
    ...
    public function fetch_properties_search(Request $request) {
    	if(isset($_POST) && !empty($_POST)){
    		...
    		if(empty($data['Selected_Min_Price']) && empty($data['Selected_Max_price'])){
    			$data['Selected_Max_price'] = Properties::where('is_deleted', '0')->max('monthly_rent');
        		$data['Selected_Min_Price'] = Properties::where('is_deleted', '0')->min('monthly_rent');
            }
			session(['Selected_Min_Price' => $data['Selected_Min_Price']]);
			session(['Selected_Max_price' => $data['Selected_Max_price']]);
    		Session::save();
    		$data['search_input'] = trim($data['search_input']);
    		if(!empty($data['search_input'])) {
				$property_list = Properties::with(['property_images','property_amenities' =>function($query) {
 				...
 				},'favorite_property' =>function($query) use ($user) {
     				if(!empty($user)){
 				...
 				'property_amenities.amenities'])->where($where)->whereBetween('monthly_rent',[$data['Selected_Min_Price'],$data['Selected_Max_price']])->where('city', 'like', '%'.$data['search_input'].'%')->orderBy('id', 'DESC')->paginate(9);
            } else{
            	$property_list = Properties::with(['property_images','property_amenities' =>function($query) {
     				return $query->where('ammenties_type','Property');
 				}
            ...
            if(!empty($data['search_input'])) {
				$count = Properties::with(['property_images','property_amenities' =>function($query) {
 				...
 				'property_amenities.amenities'])->where($where)->whereBetween('monthly_rent',[$data['Selected_Min_Price'],$data['Selected_Max_price']])->where('city', 'like', '%'.$data['search_input'].'%')->orderBy('id', 'DESC')->count();
            } else{
            	$count = Properties::with(['property_images','property_amenities' =>function($query) {
     				return $query->where('ammenties_type','Property');
 				},
	        ...
	        $notification = array(
	            ...
	            'count_property' => count($property_list)
	        );
		}        
	...
	public function appointment_list(Request $request, $property_id){
		...
		$appointments_list = Property_availabilities::where('property_id', $property_id)->whereDate('date', '>=', date('Y-m-d'))->where('time_from', '>=', date('h:i A'))->orderBy('date','asc')->paginate(4);
    ...
    public function get_appointment_dates(Request $request) {
		...
		if (!empty($_POST)) {
            ...
            else 
            {
				...
				$check_from_date = $request->avail_date." ".$from_time;
				$check_from_to = $request->avail_date." ".$appointments->time_to;
				
				$i=0;
				while ( strtotime($check_from_date) <= strtotime($check_from_to) ) 
				{
					$newDate = strtotime($check_from_date) + 60*60;
					$newDateTimeFormat = date('Y-m-d h:i A', $newDate);
					
					$time = $from_time;
					if(strtotime($time) < strtotime($appointments->time_to))
					{
						$remain_time = strtotime($appointments->time_to) - strtotime($from_time);
						...
						$from_time = $time;
						$check_from_date = $newDateTimeFormat;
					} 
					else
					{
